...
|---|---|
| `segments_blanks_mismatch.jsonl` | `segments.length != blanks.length + 1` |
| `duplicate_blank_id.jsonl` | duplicate `blank_id` inside one item |
| `position_not_consecutive.jsonl` | positions do not start at 1 or skip a value |
| `empty_accepted_fills.jsonl` | a blank has no accepted fills |
| `missing_claim_scope.jsonl` | current-schema item has no claim scope |
...
| Fixture | Expected failure |
|---|---|
| `content_pass_near_miss.jsonl` | `fill_class = near_miss` with `content_pass = true` |
| `parse_fail_with_blank_parse_pass.jsonl` | `parse_fail = true` with `blank_parse_pass = true` |
| `strict_pass_inconsistent.jsonl` | `item_strict_pass = true` despite failed blank or format |
| `format_fail_v0.jsonl` | blank-level `fill_class = format_fail` in v0 |
...
| Fixture | Expected failure |
|---|---|
| `object_fill_distribution.json` | old object-keyed fill distribution format |
| `summary_wrong_counts.json` | counts or rates do not match raw result records |
...
| Fixture | Expected failure |
|---|---|
| `wrong_file_hash.json` | one listed file SHA-256 does not match raw bytes |
| `wrong_package_hash.json` | per-file hashes match but `package_hash` is wrong |
...
| Fixture | Expected failure |
|---|---|
| `missing_one_model_repo.toml` | `policy.one_model_repo` missing or false |
| `mixed_model_submission/` | submission model differs from `model.toml` |
